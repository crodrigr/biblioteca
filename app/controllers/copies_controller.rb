class CopiesController < ApplicationController
  ...
  # GET /copies.json
  def index
    if params[:idBook]!=nil
       id=params[:idBook] 
       puts "id index: #{id}"
       session[:idBooK]=id
       puts "session[:idBooK] index: #{session[:idBooK]}"       
    else
       id=session[:idBooK]
    end
    @book=Book.find(id)
  ...
  def create
    ...
    puts "id_booksssssssssssssssssssss: #{session[:idBooK]}"    
    @copy.book_id=session[:idBooK]    
  ...
  private
    ...
    def copy_params
      params.require(:copy).permit(:codigo, :localizacion)
    end
end
